...
for z in range(1):
    ...
    for c_part in chain:
        ...
        new_dg = nx.Graph()
        new_dg.add_edges_from(
            list(
                {
                    (c_part.assignment[x[0]], c_part.assignment[x[1]])
                    for x in c_part["cut_edges"]
                }
            )
        )
    ...
    for y in range(4):
        ...
        plt.plot([0.5, 41], [0.5, 0.5], color="green", label="50%")
    ...
    for y in range(4):
        ...
        sns.distplot(
            ...
            kde=False,
            color="slateblue",
            bins=[x for x in range(10, 25)],
            hist_kws={"rwidth": 1, "align": "left"},
        )
        plt.axvline(
            x=sum([val > 0.5 for val in p_vecs[y]]),
            color="r",
            label="Current Plan",
            linewidth=5,
        )
        plt.axvline(
            x=np.mean(partisan_w[y]), color="g", label="Ensemble Mean", linewidth=5
        )
        plt.legend()
    ...
    with open("./Outputs/values/Ensemble_" + types[z] + ".txt", "w") as f:
        ...
        for y in range(4):
            ...
            f.write(
                ...
                + p_types[y]
                + ": "
                + str(sum([val > 0.5 for val in p_vecs[y]]))
            )
            f.write("\n")
            f.write(
                "Ensemble Average Wins : "
                + p_types[y]
                + ": "
                + str(np.mean(partisan_w[y]))
            )
            f.write("\n")
            f.write("\n")
